...
class Custom_Woocommerce_Subscription_Status
{
    ...
    /**
     * Registered new status by adding `Dormant` to $registered_statuses array.
     *
     * @access public
     *
    ...
    {
        $registered_statuses['wc-dormant'] = _nx_noop('Dormant <span class="count">(%s)</span>', 'Dormant <span class="count">(%s)</span>', 'post status label including post count', 'custom-wcs-status-texts');
		$registered_statuses['wc-inactive'] = _nx_noop('InActive <span class="count">(%s)</span>', 'InActive <span class="count">(%s)</span>', 'post status label including post count', 'custom-wcs-status-texts');
        return $registered_statuses;
    }

    /**
     * Add new status `Dormant` to $subscription_statuses array.
     *
     * @access public
     *
    ...
    {
        $subscription_statuses['wc-dormant'] = _x('Dormant', 'Subscription status', 'custom-wcs-status-texts');
		$subscription_statuses['wc-inactive'] = _x('InActive', 'Subscription status', 'custom-wcs-status-texts');
    ...
    public function extends_can_be_updated($can_be_updated, $new_status, $subscription)
    {
        if ($new_status == 'dormant') {
            if ($subscription->payment_method_supports('subscription_suspension') && $subscription->has_status(array('active', 'pending', 'on-hold','inactive'))) {
                $can_be_updated = true;
            } else {
		...
		elseif ($new_status == 'inactive') {
            if ($subscription->payment_method_supports('subscription_suspension') && $subscription->has_status(array('active', 'pending', 'on-hold', 'dormant'))) {
                $can_be_updated = true;
            } else {
                $can_be_updated = false;
            }
     ...
     * This code will also activate `reactivate` link in the list page for the subscription with `Dormant` status
     *
     * @access public
     *
     * @param boolean $can_be_updated default value if the current subscription can be updated to new status or not.
    ...
    {
        if ($subscription->payment_method_supports('subscription_reactivation') && $subscription->has_status(array('on-hold', 'dormant','inactive'))) {
            $can_be_updated = true;
     ...
     * This function replaces the default code with the new one.
     * This code will also activate `suspend` link in the list page for the subscription with `Dormant` status
     *
     * @access public
     *
     * @param boolean $can_be_updated default value if the current subscription can be updated to new status or not.
    ...
    {
        if ($subscription->payment_method_supports('subscription_suspension') && $subscription->has_status(array('active', 'pending', 'dormant','inactive'))) {
            $can_be_updated = true;
        } else {
            $can_be_updated = false;
        }
    ...
    public function extends_update_status($subscription, $new_status, $old_status)
    {
        if ($new_status == 'dormant') {
			...
			wcs_make_user_active($subscription->customer_user);
...
wcs_maybe_make_user_inactive($subscription->customer_user);
    ...
    {
        $bulk_actions['dormant'] = _x('Mark Dormant', 'an action on a subscription', 'custom-wcs-status-texts');
		$bulk_actions['inactive'] = _x('Mark InActive', 'an action on a subscription', 'custom-wcs-status-texts');
        return $bulk_actions;
    }
